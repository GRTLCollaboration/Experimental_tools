...
int set_grids(Vector<DisjointBoxLayout> &vectGrids, PoissonParameters &a_params)
{
    ...
    vectRHS[0] = new LevelData<FArrayBox>(vectGrids[0], NUM_CONSTRAINTS_VARS, IntVect::Zero);
    ...
    while (moreLevels)
    {
        ...
        for (int level = 0; level <= topLevel; level++)
        {
            ...
            temp_dpsi = new LevelData<FArrayBox>(vectGrids[level], NUM_CONSTRAINTS_VARS,
                                                 3 * IntVect::Unit);
        ...
        for (int lev = 1; lev <= topLevel; lev++)
        {
            ...
            delete vectRHS[lev];
            vectRHS[lev] =
                new LevelData<FArrayBox>(vectGrids[lev], NUM_CONSTRAINTS_VARS, IntVect::Zero);
