...
void GRAMRLevel::postTimeStep()
{
    ...
    str << "Intermediate_" << m_time << "_iteration_";
    m_gr_amr.writeIntermediateCheckpointFile(str.str());

    m_step_finished = true;
...
void GRAMRLevel::writeCheckpointLevel(HDF5Handle &a_handle) const
{
    ...
    // We need to output a Taylor poly if there exists a coarser level that is
    // at a different time
    bool save_taylor_poly = (m_coarser_level_ptr != nullptr) &&
                            (m_time != m_coarser_level_ptr->time());
    if (save_taylor_poly)
        header.m_int["gotFullTaylorPoly"] =
            m_patcher.getTimeInterpolator().gotFullTaylorPoly();

    // This information is important for intermediate restarts (to make sure
    // postTimeStep is called correctly)
    header.m_int["step_finished"] = m_step_finished;
    ...
    if (save_taylor_poly)
    {
        const LevelData<FArrayBox> &taylor_coeffs =
            m_patcher.getTimeInterpolator().getTaylorCoeffs();
        write(a_handle, taylor_coeffs, "taylor_coeffs",
              taylor_coeffs.ghostVect());
    }
}
...
void GRAMRLevel::readCheckpointLevel(HDF5Handle &a_handle)
{
    ...
    // Read step_finished (this is important for intermediate checkpoints, e.g.
    // to tell the difference between  intermediate checkpoints and
    // non-intermediate ones.)
    if (header.m_int.find("step_finished") == header.m_int.end())
    {
        MayDay::Warning("GRChombo::readCheckpointLevel: file does not contain "
                        "step_finished ... assuming step_finished = true");
        m_step_finished = true;
    }
    else
    {
        m_step_finished = header.m_int["step_finished"];
    }
    ...
    if (m_coarser_level_ptr != nullptr)
    {
        ...
        // read time interpolation polynomials  if it is required
        if (m_time != coarser_gr_amr_level_ptr->time())
        {
            if (header.m_int.find("gotFullTaylorPoly") == header.m_int.end())
            {
                MayDay::Error("GRChombo::readCheckpointLevel: file does not "
                              "contain gotFullTaylorPoly");
            }
            bool gotFullTaylorPoly = header.m_int["gotFullTaylorPoly"];

            LevelData<FArrayBox> &taylor_coeffs =
                m_patcher.getTimeInterpolator().setTaylorCoeffs(
                    gotFullTaylorPoly);
            DisjointBoxLayout coarsened_grid;
            coarsen(coarsened_grid, level_domain, m_ref_ratio);
            int taylor_status =
                read<FArrayBox>(a_handle, taylor_coeffs, "taylor_coeffs",
                                coarsened_grid, taylor_coeffs.interval(), true);
            if (taylor_status != 0)
                MayDay::Error("GRChombo::readCheckpointLevel: file does not "
                              "contain taylor polynomials for interpolation");
        }
    }
...
bool GRAMRLevel::step_finished() { return m_step_finished; }
